...
void validate_detector_flip(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    if(0==strcmp(type, "coco")){
        ...
        for(j = 0; j < classes; ++j){
            snprintf(buff, 1024, "%s/%s%s.txt", prefix, outfile, names[j]); //create position_file with classes
            fps[j] = fopen(buff, "w");
        }
    }
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "test1_.txt");
    char *name_list = option_find_str(options, "names", "data/names.list");
    char *prefix = option_find_str(options, "results", "results");
    char **names = get_labels(name_list);
    char *mapf = option_find_str(options, "map", 0);
...
//这个函数是网上的，出现Enter Image Path：后面输入你的txt文件路jing
...
************************************************/
            ...
            int m = plist->size;
            if(access("/home/idolphint/darknet/predictions",0)==-1)//"/home/FENGsl/darknet/data"修改成自己的路径
            {
              if (mkdir("/home/idolphint/darknet/predictions",0777))//"/home/FENGsl/darknet/data"修改成自己的路径
               {
        ...
        if(outfile){
            save_image(im, outfile);
             ...
             char b[2048];
            sprintf(b,"/home/idolphint/darknet/predictions/%s",GetFilename(path));//"/home/FENGsl/darknet/data"修改成自己的路径
